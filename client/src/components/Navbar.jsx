import { useState } from 'react';
import { HiMenuAlt4 } from 'react-icons/hi';
import { AiOutlineClose } from 'react-icons/ai';
import { useNavigate } from 'react-router-dom'

import logo from '../../images/logo.png';

const NavbarItem = ({ title, classProps }) => {

    const openInNewTab = (url) => {
        const newWindow = window.open(url, '_blank', 'noopener,noreferrer')
        if (newWindow) newWindow.opener = null
    }

    const onHandleClickNavbarItem = () => {
        if(title === 'Tutorials'){
            openInNewTab('https://www.coinbase.com/learn/tips-and-tutorials')
        }

        if(title === 'Buy ETH'){
            openInNewTab('https://app.uniswap.org/#/swap?chain=mainnet')
        }

    }

    


    return (
        <li className={`mx-4 cursor-pointer ${classProps}`} onClick={() => onHandleClickNavbarItem()}>
            {title}

        </li>
    );
}

const Navbar = () => {
    const [toggleMenu, setToggleMenu] = useState(false);
    return (
        <nav className='border-bottom-wishnee w-full flex md:justify-center justify-between items-center p-4 ' >
            <div className="md:flex-[0.5] flex-initial justify-center items-center">
                <img src={logo} alt="logo" className='w-40 cursor-pointer' />
            </div>
            <ul className="md:flex hidden list-none flex-row justify-between items-center flex-initial">
                {["Buy ETH", "Tutorials", "News"].map((item, index) => (
                    <NavbarItem key={item + index} title={item} />
                ))}

                <li className='text-white bg-[#BE1931] py-2 px-7 mx-4 rounded-full curdor-pointer hover:bg-[#CE1931]'>
                    <button>Login</button>
                </li>
            </ul>
            <div className='flex-relative'>
                {toggleMenu
                    ? <AiOutlineClose fontSize={28} className="md:hidden cursor-pointer" onClick={() => setToggleMenu(false)} />
                    : <HiMenuAlt4 fontSize={28} className="md:hidden cursor-pointer" onClick={() => setToggleMenu(true)} />
                }

                {toggleMenu && (
                    <ul className='z-10 fixed top-0 -right-2 p-3 w-[70vw] h-screen shadow-2xl md:hidden list-none
                    flex flex-col justify-start items-end rounded-md red-glass-style  animate-slide-in'>
                        <li className='text-xl w-full my-2'>
                            <AiOutlineClose onClick={() => setToggleMenu(false)} />
                        </li>
                        {["Buy ETH", "Tutorials", "News"].map((item, index) => (
                            <NavbarItem key={item + index} title={item} classProps="my-2 text-lg" />
                        ))}
                    </ul>
                )}
            </div>

        </nav>
    );
}

export default Navbar;